...
breed [hunters hunter] ; cria agentes do tipo caçador


turtles-own [ energy ]
leoes-own [descanso_ticks ticks-sem-comer]
...
to MoveHunters
  ask hunters [
    ; Verifica se há leões ou hienas em um raio de 2 patches
    let nearby-leoes leoes in-radius 2
    let nearby-hienas hienas in-radius 2
    ifelse any? nearby-leoes [
      ; Se houver leões próximos, mate um deles
      let target-leao one-of nearby-leoes
      ask target-leao [
        ask patch-here [set pcolor brown] ; Muda a cor do patch onde o leão está
        die
      ]
    ] [
      ; Se não houver leões, mas houver hienas, mate uma delas
      ifelse any? nearby-hienas [
        let target-hiena one-of nearby-hienas
        ask target-hiena [
          ask patch-here [set pcolor brown] ; Muda a cor do patch onde a hiena está
          die
        ]
      ] [
...
to MoveLeoes
  ask leoes [
       let hienas_frente count hienas-on patch-ahead 1
       let hienas_direita count hienas-on patch-right-and-ahead 90 1
       let hienas_esquerda count hienas-on patch-left-and-ahead 90 1

       print(word "olaf" hienas_frente)
       print(word "olad" hienas_direita)
       print(word "olae" hienas_esquerda)

     set ticks-sem-comer ticks-sem-comer + 1

    if ticks-sem-comer >= 10[
      set energy energy * 0.95
      set ticks-sem-comer 0
    ]
...
to Leoes-Armadilha
   ...
   if [pcolor] of patch-ahead 1 = magenta or  [pcolor] of patch-right-and-ahead 90 1 = magenta or  [pcolor] of patch-left-and-ahead 90 1 = magenta[
      set energy energy - (energy * 0.2) ; perde 20% de energia
    ]
